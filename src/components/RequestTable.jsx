          ...
          <tbody>
            {sortedData().map((row, index) => (
              <tr key={index}>
                ...
                <td>{row.phone}</td>
                <td>
                  <button className="flex flex-column btn btn-outline btn-primary float-right" onClick={()=>document.getElementById(`infoModal_${row.student_id}`).showModal()}>
                  ...
                  <RequestModal key={index} student_id={`infoModal_${row.student_id}`} user_id={row.userid} info={row.name} requestType={requestType} admin={admin}/>
                </td>
              </tr>
            ))}
          </tbody>
